                            ...
                            <tbody class="divide-y divide-gray-50">
                                @foreach($mealplan as $tr)
                                    <tr class="hover:bg-indigo-50/50 transition-all duration-300 group">
                                        ...
                                        {{-- التقييم الرقمي --}}
                                        <td class="px-6 py-5 bg-white group-hover:bg-transparent text-center border-y border-gray-50 group-hover:border-transparent">
                                            <div class="inline-flex items-center px-3 py-1 bg-yellow-50 border border-yellow-100 rounded-lg shadow-sm">
                                        ...
                                        <td class="px-6 py-5 bg-white group-hover:bg-transparent border-y border-gray-50 group-hover:border-transparent">
                                            <div class="max-w-xs overflow-hidden">
                                                <p class="text-gray-600 text-sm leading-relaxed italic border-r-2 border-indigo-100 pr-3">
                                                    "{{ $tr->comment ?? 'لا يوجد تعليق مكتوب من قبل المتدرب' }}"
                                                </p>
                                            </div>
                                        </td>

                                        {{-- التقييم بالنجوم --}}
                                        <td class="px-6 py-5 bg-white group-hover:bg-transparent rounded-l-2xl text-center border-y border-l border-gray-50 group-hover:border-transparent">
                                            <div class="flex items-center justify-center gap-0.5">
                                                @for($i = 1; $i <= 5; $i++)
                                                    <svg class="w-4 h-4 {{ $i <= $tr->rating ? 'text-yellow-400' : 'text-gray-200' }} fill-current transition-colors duration-200" viewBox="0 0 20 20">
                                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                                    </svg>
                                                @endfor
                                            </div>
                                            <div class="mt-1 text-[9px] font-black text-gray-400 uppercase tracking-tighter">
                                                {{ $tr->rating }} / 5 نجوم
                                            </div>
                                        </td>
                                    </tr>
